namespace ArayCode.Formatting;

/// <summary>
/// Contains the complete Spectre.Console color palette.
/// </summary>
public static class SpectreColorPalette
{
    /// <summary>
    /// HashSet of all known Spectre.Console color names and style keywords.
    /// Uses OrdinalIgnoreCase for case-insensitive matching.
    /// </summary>
    private static readonly HashSet<string> _knownColors = new(System.StringComparer.OrdinalIgnoreCase)
    {
        // ── Decoration keywords ──────────────────────────────────────
    ...
    /// <summary>
    /// Checks if a color name is valid in the Spectre.Console palette.
    /// </summary>
    /// <param name="colorName">The color name to check (case-insensitive).</param>
    /// <returns>True if the color is known, false otherwise.</returns>
    public static bool IsValidColor(string? colorName)
    {
        if (string.IsNullOrEmpty(colorName))
            return false;
        return _knownColors.Contains(colorName);
    }
}
